document.addEventListener('DOMContentLoaded', async function () {
    try {
        ...
        // Fetch group details
        const groupDetailsResponse = await axios.get(`http://13.233.193.120//group/showGroupDetails/${groupId}`, {
...
async function setupSearch() {
    ...
    searchInput.addEventListener('input', async function (event) {
        ...
        try {
            ...
            const searchResult = await axios.get(`http://13.233.193.120//group/searchUsersByUsername?searchTerm=${searchTerm}`,  {
                headers: { "Authorization": token }
            });
...
async function addMember(userId) {
    ...
    try {
        const response = await axios.post(`http://13.233.193.120//group/${groupId}/add-member`, 
...
async function removeMember(memberId) {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await axios.delete(`http://13.233.193.120//group/${groupId}/remove-member/${memberId}`, {
            headers: { "Authorization": token }
        });
        console.log("Response from server:", response.data);
...
//     const token = localStorage.getItem('token');
//     try {
//         await axios.put(`http://13.233.193.120//group/${groupId}/make-admin`,
//         { memberId },
//         {
//             headers: { "Authorization": token }
//         });
