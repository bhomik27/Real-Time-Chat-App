document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function fetchAndDisplayGroupChats(groupId) {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://13.233.193.120//chat/getallchatsofgroup/${groupId}`, { headers: { "Authorization": token } });
    ...
    async function sendMessage(groupId, userId, message) {
        try {
            ...
            const response = await axios.post(
                `http://13.233.193.120//chat/createmessage/${groupId}/${userId}`,
                { message },
